...
const getChartConfig = (period) => {
  ...
  if (period === 'Last 90 Days') return { apiPeriod: 'daily', days: 90, bucket: 'day' };
...
const RedemptionAnalytics = () => {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      setConversionRate(users.length > 0 ? ((redemptions.length / users.length) * 100).toFixed(1) : '0.0');
  ...
  if (loading) return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}><Skeleton style={{ height: 300 }} /><Skeleton style={{ height: 300 }} /></div>
      <Skeleton style={{ height: 400 }} />
    </div>
  );
  ...
  if (error) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '80px 0', gap: 16, backgroundColor: C.white, borderRadius: 16, border: '1px solid #fee2e2' }}>
      ...
      <button onClick={fetchAll} style={{ padding: '10px 24px', borderRadius: 8, border: 'none', backgroundColor: C.primary, color: C.white, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>Retry</button>
    </div>
  );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 28, fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 16 }}>
        ...
        <button onClick={() => navigate('/admin/dashboard')} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 600, color: C.primary, backgroundColor: 'transparent', border: 'none', cursor: 'pointer' }}>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 20 }}>
        <div style={{ ...baseCard, borderRadius: 16, padding: 24, cursor: 'default' }} {...hoverCard}>
          <div style={{ padding: 10, borderRadius: 12, backgroundColor: C.primarySoft, color: C.primary, display: 'inline-flex', marginBottom: 20 }}>
          ...
          <h3 style={{ fontSize: 36, fontFamily: 'Poppins, sans-serif', fontWeight: 700, color: C.primary, lineHeight: 1, margin: 0 }}>{totalRedemptions.toLocaleString()}</h3>
          <p style={{ fontSize: 12, color: C.outline, marginTop: 8 }}>All time historical data</p>
        </div>

        <div style={{ ...baseCard, borderRadius: 16, padding: 24, cursor: 'default' }} {...hoverCard}>
          <div style={{ padding: 10, borderRadius: 12, backgroundColor: C.goldSoft, color: C.goldDark, display: 'inline-flex', marginBottom: 20 }}>
          ...
          <h3 style={{ fontSize: 36, fontFamily: 'Poppins, sans-serif', fontWeight: 700, color: C.primary, lineHeight: 1, margin: 0 }}>{formatLargeNumber(totalPointsRedeemed)}</h3>
          <p style={{ fontSize: 12, color: C.outline, marginTop: 8 }}>Cumulative points spent</p>
        </div>

        <div style={{ ...baseCard, borderRadius: 16, padding: 24, cursor: 'default' }} {...hoverCard}>
          <div style={{ padding: 10, borderRadius: 12, backgroundColor: C.tealSoft, color: C.teal, display: 'inline-flex', marginBottom: 20 }}>
          ...
          <h3 style={{ fontSize: 36, fontFamily: 'Poppins, sans-serif', fontWeight: 700, color: C.primary, lineHeight: 1, margin: 0 }}>{avgPointsPerRedemption.toLocaleString()}</h3>
          <p style={{ fontSize: 12, color: C.outline, marginTop: 8 }}>Cost efficiency metric</p>
        </div>

        <div style={{ ...baseCard, borderRadius: 16, padding: 24, cursor: 'default' }} {...hoverCard}>
          <div style={{ padding: 10, borderRadius: 12, backgroundColor: C.purpleSoft, color: C.purple, display: 'inline-flex', marginBottom: 20 }}>
          ...
          <h3 style={{ fontSize: 36, fontFamily: 'Poppins, sans-serif', fontWeight: 700, color: C.primary, lineHeight: 1, margin: 0 }}>{conversionRate}%</h3>
          <p style={{ fontSize: 12, color: C.outline, marginTop: 8 }}>{totalUsers} total users base</p>
        </div>
      </section>
      ...
      <section style={{ ...baseCard, borderRadius: 16, padding: 28, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', position: 'relative', minHeight: 300 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: chartPeriod === 'Last 90 Days' ? 4 : (isYtd ? 8 : 12), height: 280, position: 'relative', zIndex: 1, overflowX: 'auto', paddingBottom: 4, minWidth: isYtd ? chartBars.length * 48 : (chartPeriod === 'Last 90 Days' ? chartBars.length * 16 : '100%') }}>
            {chartBars.map((bar, i) => (
              <div key={i} className="pp-bar-wrap" style={{ flex: 1, height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', position: 'relative', minWidth: isYtd ? 40 : (chartPeriod === 'Last 90 Days' ? 12 : 'auto') }}>
                ...
                />
                <span style={{ fontSize: (isYtd || chartPeriod === 'Last 90 Days') ? 9 : 12, fontWeight: bar.highlight ? 700 : 500, marginTop: 12, color: bar.highlight ? C.onSurface : C.outline, whiteSpace: 'nowrap' }}>
                  {bar.label}
                </span>
              </div>
            ))}
      ...
      <section style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                ...
                return (
                  <div key={cat.name}>
                    ...
                    <div style={{ width: '100%', height: 8, borderRadius: 99, backgroundColor: C.surfaceHigh, overflow: 'hidden' }}>
                      <div style={{ width: `${pct}%`, height: '100%', borderRadius: 99, backgroundColor: cat.rank === 1 ? C.gold : C.primarySoft, transition: 'width 1s ease' }} />
                    </div>
                  </div>
                );
              })}
        ...
        {/* Insights Box */}
        <div style={{ ...baseCard, borderRadius: 16, padding: 24, background: `linear-gradient(135deg, ${C.primary} 0%, ${C.primaryContainer} 100%)`, color: C.white, border: 'none' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ padding: '12px 16px', backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 10 }}>
              ...
              <div style={{ padding: '12px 16px', backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 10 }}>
                ...
                <p style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>{conversionRate}% <span style={{ fontSize: 12, fontWeight: 400, opacity: 0.8 }}>of users redeemed</span></p>
              </div>
              <div style={{ padding: '12px 16px', backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 10 }}>
      ...
      <section style={{ ...baseCard, borderRadius: 16, overflow: 'hidden' }}>
        {/* ✅ UPDATED: Added Button to Header */}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 600 }}>
            ...
            <tbody>
              {recentRedemptions.length === 0 ? (
                <tr><td colSpan={5} style={{ textAlign: 'center', padding: '60px 20px', color: C.outline }}>No redemptions recorded yet</td></tr>
